...
bool vm_alloc_page_with_initializer(enum vm_type type, void *upage,
  ...
  if (spt_find_page(spt, upage) == NULL) {
    ...
    list_push_back(&victims, &page->victims_elem);
...
static struct frame *vm_get_victim(void) {
  ...
  // printf("@@ vm_Get_victim 실행\n");

  if (list_empty(&victims)) return NULL;

  // printf("@@ vm_get_victim() for loop START !!\n");

  // printf("victim cnt : %d\n", list_size(&victims));

  for (e = list_begin(&victims); e != list_end(&victims); e = list_next(e)) {
    page = list_entry(e, struct page, victims_elem);
    ...
    if ((page->owner == NULL) ||
        !pml4_is_accessed(&page->owner->pml4, page->va)) {
      // printf("@@ if(pml4 accessed !)page->va = %p\n", page->va);
      // if (!page->frame) {
  ...
  // printf("for loop DOEN ! victim is %p\n", victim->page->va);
...
static struct frame *vm_evict_frame(void) {
  ...
  // lock_acquire(&victims_lock);
  victim = vm_get_victim();
  // lock_release(&victims_lock);
  /* TODO: swap out the victim and return the evicted frame. */
  // printf("@@ victim is %p\n", victim->page->va);
  // printf("---- vm_evict_frame() find victim kva : %p\n", victim->kva);
  if (!swap_out(victim->page)) {
    PANIC("---- swap out fail !!\n");
    return NULL;
  }
  victim->page = NULL;
  // printf("---- evict done !! victim kva : %p\n", victim->kva);
  // printf("---- evict done !! victim kva : %p\n", victim->kva);
  return victim;
}
...
bool vm_try_handle_fault(struct intr_frame *f UNUSED, void *addr UNUSED,
  ...
  if (!page)  // 페이지가 없을 때
  {
    ...
    {
      // printf("---- no page & call vm_stack_growth() !! \n");

      vm_stack_growth(addr);  // 스택 확장
      return true;            // 스택 확장 성공
    }

    // printf("---- no page & no stack_growth(), addr : %d \n");
    ...
    if (VM_TYPE(parent_type) == VM_UNINIT) {
      ...
      if ((parent_type & flag) == flag) {
        ...
        if (do_mmap(parent_page->va, parent_page->file.size,
                    parent_page->writable, new_file,
                    parent_page->file.ofs) == NULL) {
          return false;
        }
        continue;
      }
