...
static bool file_backed_swap_in(struct page *page, void *kva) {
  ...
  printf("---- file_backed swap in() 호출 frame kva : %p, page->va : %p\n", kva,
         page->va);

  // bool result = lazy_load_segment(page, NULL);

  // printf("---- file_backed swap_in() succ : %s\n", result ? "true" : "false");
...
static bool file_backed_swap_out(struct page *page) {
  ...
  // if (page->file.owner != t) return false;
  // printf("---- swap out call \n");
...
void *do_mmap(void *addr, size_t length, int writable, struct file *file,
  ...
  while (0 < remain_size) {
    ...
    if (!vm_alloc_page_with_initializer(VM_FILE | VM_MARKER_1, upage, writable,
